...
def dataread():
	wladdress = "./wl.csv"
	placingsaddress = "./placings.csv"
	metadataaddress = "./metadata.csv"

	metafile = open(metadataaddress, "r")
	metadata = []
	i = 0
	for line in metafile:
		if i > 0:
			linedat = line.split("\n")
			linedata = linedat[0].split(",")
			metadata.append(linedata[1:])
		i += 1
	meta = np.array(metadata)
	metafile.close()

	wlfile = open(wladdress,"r")
	wins = np.zeros((67,67))
	i = 0
	for line in wlfile:
		if i > 0:
	...
	placefile = open(placingsaddress, "r")
	i = 0
	tourneys = []
	placings = np.zeros((67,27))
		...
		for cell in line.split(","):
			...
			if i > 1 and j > 1:
				if cell.split(",")[0] == '':
					...
					if cell[0] != "\n":
						placings[i-2,j-2] = cell
					else:
						placings[i-2,j-2] = -1
			if i == 0 and j > 1 and cell[0] != '\n':
				tourneys.append(int(cell))
			if i == 1 and j > 1 and cell[0] != '\n':
				tourneys[j-2] = [tourneys[j-2],cell.split("\n")[0]]
			j += 1
		i += 1

	return (wins, meta, np.array(tourneys, dtype='object'), placings)

# returns the number of rounds that a player played through, given
# their final tournament placing. Counts losers rounds only (as if
# the player lost WR1 winners)
def prounds(place, nent):
	if not(type(place) is type(0.4) or type(place) is type(0)) or place < 0:
		return "N/A"
	nrounds = float(ceil(log(nent, 2)) + ceil(log(log(nent,2),2)))
	if place == 1: # top 1
		return 1.
	if place == 2: # top 2
		return 1. - 1./nrounds
...
def debug():
	#for i in range(50):
		#a = np.array([i,8.,55.],dtype=np.float64)
		#print i, np.log(1./(1.+np.exp(a[1]*(a[0]+a[2]))))
